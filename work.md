...
#### Carga 
En primer lugar guardamos la dirección en la que se guardaran los archivos y nos aseguramos de que tengan permiso de escritura. Tras esto empezamos a trabajas con el archivo, conprobamos (1º if) que el envio es por POST. El primer paso es copmrobar si hay archivo
```php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $file = $_FILES[$fieldName];    //SIMPLIFICAL EL ACCESO A LA INFORMACIÓN
    if ($file['error'] !== UPLOAD_ERR_OK) {   // Y NO $_FILES[$fieldName]['error]
    ...
    $fileName = basename($file['name']);  //elimina una posible ruta original del doc 
